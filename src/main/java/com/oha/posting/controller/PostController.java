...
public class PostController {
    ...
    @GetMapping("/posts")
    @Operation(summary = "게시물 리스트 조회", description = """
                                                          **statusCode:**
                                                          - 200: 성공
                                                          - 400: 데이터 오류
                                                          - 404: 게시물 없음
    ...
    @Operation(summary = "게시물 작성", description = """ 
                                                    **Status Code:**
                                                    - `201`: 성공
                                                    - 400: 데이터 오류
                                                    - 404: 게시물 없음
    ...
    public ResponseObject<PostInsertResponse> insertPost(@RequestPart(name = "dto") @Validated PostInsertRequest dto
        ...
        return postService.insertPost(dto, files, token, userId, httpServletResponse);
    }
    ...
    @Operation(summary = "게시물 좋아요", description = """
                                                **statusCode:**
                                                - 200: 성공(좋아요 취소)
                                                - `201`: 성공(좋아요)
                                                - 400: 데이터 오류
    ...
    public ResponseObject<?> like(@Parameter(hidden = true) @RequestHeader(name = "x-user-id") Long userId
                                ...
                                , HttpServletResponse httpServletResponse) throws Exception {
        return postService.like(userId, dto, httpServletResponse);
    }

    @PostMapping("/post/report")
    @Operation(summary = "게시물 신고하기", description = """
                                                **statusCode:**
                                                - `201`: 성공
                                                - 400: 데이터 오류
                                                - 404: 게시물 없음
    ...
    public ResponseObject<?> reportPost(@Parameter(hidden = true) @RequestHeader(name = "x-user-id") Long userId
                                      , @RequestBody @Validated PostReportRequest dto) throws Exception {
